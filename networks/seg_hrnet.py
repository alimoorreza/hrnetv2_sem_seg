...
class HighResolutionNet(nn.Module):
    ...
    def init_weights(self, load_imagenet_model: bool=False, imagenet_ckpt_f_path: str='') -> None:
        ...
        if not load_imagenet_model:
            return
        if os.path.isfile(imagenet_ckpt_f_path):
            pretrained_dict = torch.load(imagenet_ckpt_f_path)
            logger.info('=> loading pretrained model {}'.format(imagenet_ckpt_f_path))
            ...
            raise Exception('no pretrained model found at {}'.format(imagenet_ckpt_f_path))
        

def get_seg_model(cfg, criterion, n_classes, load_imagenet_model, imagenet_ckpt_f_path, **kwargs):
    model = HighResolutionNet(cfg, criterion, n_classes, **kwargs)
    model.init_weights(load_imagenet_model, imagenet_ckpt_f_path)
    assert isinstance(model, nn.Module)
    return model


def get_configured_hrnet(n_classes, config, load_imagenet_model=False, imagenet_ckpt_f_path=''):
    criterion = nn.CrossEntropyLoss(ignore_index=config.LOSS.IGNORE_LABEL)
    model = get_seg_model(config, criterion, n_classes, load_imagenet_model, imagenet_ckpt_f_path)
    return model
